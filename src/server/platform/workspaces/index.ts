...
export async function nextWorkspaceSlug(
  env: Env,
  name: string,
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const row = await env.DB.prepare(
      ...
      .bind(slug, excludeWorkspaceId ?? null, excludeWorkspaceId ?? null)
      .first<{ id: string }>();
    if (!row) return slug;
  }
...
export async function archiveWorkspace(
  ...
  const now = Date.now();
  await env.DB.batch([
    env.DB.prepare(`UPDATE workspace SET archived_at = ?, updated_at = ? WHERE id = ?`).bind(
      now,
      now,
      workspaceId,
    ),
    env.DB.prepare(`UPDATE session SET workspace_id = NULL WHERE workspace_id = ?`).bind(
      workspaceId,
    ),
  ]);
  await audit(env, {
...
export async function transferWorkspaceOwnership(
  ...
  await env.DB.batch([
    env.DB.prepare(
      `UPDATE workspace_user SET role = 'owner' WHERE workspace_id = ? AND user_id = ?`,
    ).bind(workspaceId, targetUserId),
    env.DB.prepare(
      `UPDATE workspace_user SET role = 'admin' WHERE workspace_id = ? AND user_id = ? AND user_id != ?`,
...
export async function deleteWorkspace(
  ...
  const now = Date.now();
  await env.DB.batch([
    env.DB.prepare(`UPDATE workspace SET deleted_at = ?, updated_at = ? WHERE id = ?`).bind(
      now,
      now,
      workspaceId,
    ),
...
export async function setSessionWorkspaceFallback(
  env: Env,
  sessionId: string,
  userId: string,
...
export async function updateWorkspaceMemberRole(
  ...
  if (
    current.role === 'owner' &&
    role !== 'owner' &&
    (await countWorkspaceOwners(env, workspaceId)) <= 1
  ) {
    return 'last_owner';
  }
...
export async function removeWorkspaceMember(
  ...
  const current = await env.DB.prepare(
    ...
    .first<{ role: WorkspaceRole }>();
  if (!current) return 'not_found';
  if (current.role === 'owner' && (await countWorkspaceOwners(env, workspaceId)) <= 1)
    return 'last_owner';
...
export async function acceptWorkspaceInvitation(
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`UPDATE workspace_invitation SET accepted_at = ? WHERE id = ?`).bind(
      now,
      invitation.id,
    ),
  ]);
