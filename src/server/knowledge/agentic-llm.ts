...
const SCOPES = ['knowledge', 'resolved_tickets', 'customer_data', 'all'] as const;
...
const RewriteSchema = z.object({ query: z.string().min(1).max(500) });
...
function validScope(scope: unknown): scope is KnowledgeSearchScope {
  return typeof scope === 'string' && SCOPES.includes(scope as KnowledgeSearchScope);
}
