 ...
 * validates the retained suffix.
 */
export async function runAuditRetentionSweep(
  env: Env,
): Promise<{ deleted: number; retentionDays: number }> {
  const configured = Number((env as unknown as { AUDIT_RETENTION_DAYS?: string }).AUDIT_RETENTION_DAYS);
  const retentionDays = Number.isFinite(configured) && configured > 0 ? configured : 0;
  if (retentionDays <= 0) return { deleted: 0, retentionDays: DEFAULT_RETENTION_DAYS };
