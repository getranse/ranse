...
export async function infer(params: InferParams): Promise<InferResult> {
  ...
  if (candidates.length === 0) {
    throw new Error(
      `No usable LLM for action "${params.action}" — primary "${cfg.model}" and fallback "${cfg.fallbackModel ?? '(none)'}" both require API keys that aren't configured. Add a key under Settings → LLM providers, set the corresponding Worker secret, or change the model to a workers-ai/ option.`,
    );
  }
